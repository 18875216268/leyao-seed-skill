...
6. **凭证先验后问**：**命中资产且其路由/文档声明需要登录**时——取资产路径（**唯二来源**：`asset.py resolve <节点id>` ｜ `ROUTES.md` 的 `→ 挂载`；**禁其它形式** ✗）→ 判「**凭证已在位？**（本地缓存 ｜ 用户提供）」：在位 → 按其〈登录〉说明**验证**后使用 ✓；不在位 → 按其说明走**登录流程**（有现成登录器→**必须调登录器并等登录成功**（其自动落库）；**无 → 按对应资产或其子资产登录指引自行获取**〔完整梳理、自写自理〕）；**未验证不得断言「缺凭证」、不得向用户索要** ✗（**索要须附验证留证**）；**禁自写登录流程**（**有独立登录器时** ✗——必须经登录器工具；**无 → 按登录指引自行获取** ✓）；**有登录器时，凭证取用一律经其工具**（**禁止 AI 自行获取** ✗）；**失效 / 缺失处置：当场重走登录流程**（调登录器 · **弹窗＝协助请求** → 回验；**≤3 → 问题流程**）✓（壳自动调起＝实现之一 ✓；**登录器不可用（判定：修复环境后仍无法弹窗 / 无登录器可用）→ 换其它的合法登录方式（备用通道 / 子资产方式）→ 仍失败 → 询问用户** ✓）；**用户已明示登录方式 / 凭证来源 → 按其走** ✓；失败按 `control.md`〈卡壳处置〉〈问题未解决流程〉（≤3 重走 → 问用户）；凭证状态逐资产独立 ✗。
...
接到任务
  ...
  → 进入 processor/（按五步流程执行；每步判据自带，见 flow/1~5 与 control.md）
      执行时：命中资产则进其挂载目录读 `app.md`，**并按其指引继续向下（如 `references/` 等）直到读取执行所需全文**（或信息足够并留证；读取可用 `python library/asset.py read <相对路径>`——只读 · 附读取凭据）；**凭证类硬门**：**凡依赖 / 命中登录型资产（数据 · 工具 · 通道）→ 动手前第一动作＝按该资产 `app.md`〈登录〉说明跑验证（`--status` 类 · 只读不弹窗）**；未过才重登（**弹窗＝协助请求**）→ 回验；**未验证不得断言「缺凭证 / 需用户提供」、不得向用户索要** ✗（**索要须附验证留证**）；**「需要登录」＝我发起登录器请用户协助（凭证只在登录器内流动 ✓），≠ 让用户把凭证交给我（那样才是碰凭证）**，再原样调用；无命中按自带判据亲自动手
