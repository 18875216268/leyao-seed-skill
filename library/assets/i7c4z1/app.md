...
- **备用路径（集团方式 · 备用通道）**：自有组件不可用（`--check` 修复环境后仍无法弹窗 / 无登录器可用）或主通道能力不足时的替代路径——**启用条件与降级规则一律以 `vendor/SUBSKILL_ROUTING.md` §1/§3 为准**（用户明确要求 / 提供该通道凭证；或主通道能力不足且备用通道可用 → 自动降级、必说明原因 ✓）。**当前备用通道＝`bi-pat`（PAT SQL）**，操作细节完全遵照其文档（`vendor/bi-pat/app.md`）——**集团方式可能变化 → 以集团实时落盘文档为准 ✓**。与主通道**二选一、不并用** ✓（鉴权不混装 ✗）；**已有有效凭证时不得再发起任何登录** ✗；备用不等于转述，本框架不做任何假设。
- 用法：
...
- 当前已接入优化 skill（板块）：
  ① **BI-出库统计Ultra查询v1.08**（出库统计Ultra 板块：DSL 聚合查询/批量并发/分页/区域树/聚合导出；自包含，凭证三级窗口。详见 `vendor/optimizers/BI-出库统计Ultra查询v1.08/h4dsa6/app.md` 与 `vendor/SUBSKILL_ROUTING.md` §5）；
  ② **摇钱树业务线查询v1.0**（摇钱树业务线板块：6 看板/142 组件/41 筛选器、j837 出库日期铁律、候选值实时缓存、聚合导出；触发词「摇钱树/钱树/子公司业务线/销售进度/库存报表/补货建议/货主利润表/商务考核/经营分析」→ 本板。详见 `vendor/optimizers/摇钱树业务线查询v1.0/yqst01/app.md` 与 `vendor/SUBSKILL_ROUTING.md` §5）。
- 通道：**1 = bi-cookie（主）**· **2 = bi-pat（备，仅持 PAT 用户）**——启用条件与权限分流见 `vendor/SUBSKILL_ROUTING.md` §1 ✗。

## 2.1 数据权限隔离与选板必验（部门级 · 硬规则）

公司数据权限按**账号/项目部**隔离（运营部 / 采购部〔采购一部、采购二部＝摇钱树项目部〕互不互通）。同一 BI token，运营账号取不了项目部专属数据，项目部账号也取不了其它项目部数据——同平台同账号体系，但数据权限不互通。因此路由到任一**板块优化板**后、正式取数前，必须先验该板的访问权限：

- **选板必验**：命中板块（按 `vendor/SUBSKILL_ROUTING.md` §3/§5）→ 先跑该板权限自检（如摇钱树板 `python vendor/optimizers/摇钱树业务线查询v1.0/yqst01/scripts/probe.py`）→ `ACCESSIBLE` 才继续；`NO_PERMISSION` 则该板对当前账号不可用，需换对应项目部账号重登（`scripts/login_bi.py`）或改取用户有权限的板块，**绝不跨项目部替选**（如拿运营数据冒充项目部数据）。
- **结果三分类（务必分清，勿误判）**：BI 的无权限响应伪装成 HTTP 500 + `error_code:1004 无权访问`，不是服务端抖动——自检/查询遇到 500 必须解析响应体里的 `error_code`，不能当重试。`ACCESSIBLE` / `NO_PERMISSION` / `AUTH_EXPIRED`（token 失效，按 §4 换凭证）/ `NETWORK`（真网络错，重试）。
- **业务类型-摇钱树(YQS) ≠ 摇钱树业务线板块**：通用数据里「摇钱树」只是一个**业务类型筛选值**，用户只想按该值过滤时走**通用取数**（出库统计Ultra / `vendor/bi-cookie`，用 `业务类型=摇钱树(YQS)` 过滤），**不要**路由到摇钱树业务线板块；只有当用户要的是摇钱树项目部**专属看板**（销售进度/库存报表/补货建议/货主利润表/商务考核/经营分析）才走该板块。两者层级不同，AI 须先判断用户意图再路由。
...
- **导出下载 403（approval required）**：卡片/优化板导出链中**下载步骤**返回 403 且响应体含 `approval required` → 属**服务端导出审批拦截**（导出任务本身已成功：状态 FINISHED、服务端已生成文件）——**非账号权限问题、非本包代码缺陷**，与取数类 403（权限不足，仍按所选通道错误速查表处置 ✓）**不是同一回事**；纪律：**不重试、不换账号绕行、不改本包与 `vendor/` 任何代码或请求头** ✗；处置＝① 需数据时改走取数链路（按所选通道/优化板指引查询 + **分页翻页**逐页取全）在本地落盘 ✓；② 确需官方导出文件 → 走 BI 侧审批流程（业务/管理员侧，非本包可解）。
